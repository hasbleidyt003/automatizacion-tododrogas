...
st.set_page_config(
    page_title="Renombrador CUV - Coosalud", 
    page_icon="🔢", 
    layout="wide"
)
modern_navbar()

st.title("🔢 Renombrador CUV - Coosalud")
...
with st.expander("🔍 Ejemplos de Patrones Válidos"):
    ...
    **Patrones que SÍ serán renombrados:**
    ...
    - `NE123456789.json` → `CUV_NE123456789.json`
    - `documento_NE8888.txt` → `CUV_NE8888.txt`
    
    **Patrones que NO serán renombrados:**
    - `factura_normal.pdf`
    - `CUV_NE651.pdf` (ya está en formato correcto)
    - `NE2450-CUV.pdf` (usa el Renombrador CUV SISPRO para este formato)
    """)
...
with st.expander("📖 Instrucciones de Uso"):
    ...
    ### Diferencia con CUV SISPRO:
    - **Este renombrador**: Para archivos con formato `NE651.ext`
    - **CUV SISPRO**: Para archivos con formato `NE651-CUV.ext`
    """)

# FOOTER
st.markdown("---")
st.caption("🔢 Renombrador CUV - Coosalud • v1.0 • Conversión de patrones NE######")
