...
st.set_page_config(
    page_title="Renombrador CUV SISPRO - Coosalud", 
    page_icon="🔢", 
    layout="wide"
)
modern_navbar()

st.title("🔢 Renombrador CUV SISPRO - Coosalud")
st.markdown("Convierte archivos con patrón NE######-CUV a formato CUV_NE######")

# Función de procesamiento
def renombrar_archivos_cuv_sispro(directorio):
    resultados = []
    contador = 0
    ...
    try:
        for archivo in os.listdir(directorio):
            ...
            if os.path.isfile(ruta_completa):
                # Buscar el patrón NE seguido de números y luego -CUV
                patron = r'(NE\d+)-CUV'
                coincidencia = re.search(patron, archivo)
                
                if coincidencia:
                    numero_factura = coincidencia.group(1)  # Extraer el NE2450, NE99999999, etc.
                    
                    # Obtener la extensión del archivo
                    nombre_base, extension = os.path.splitext(archivo)
                    
                    # Crear el nuevo nombre: CUV_NE2450.ext
                    ...
                    resultados.append({
                        'original': archivo,
                        'nuevo': archivo,
                        'estado': 'ℹ No coincide con patrón NE######-CUV',
                        'tipo': 'info',
                        'numero_factura': 'N/A'
                    })
...
uploaded_files = st.file_uploader(
    "Selecciona archivos para renombrar (patrón NE######-CUV)",
    accept_multiple_files=True,
    help="Archivos con formato: NE2450-CUV.pdf, NE999999-CUV.xlsx, etc."
)

# Información adicional
st.info("""
**ℹ️ Funcionalidad:**
- Convierte archivos con patrón `NE######-CUV` a `CUV_NE######`
- Ejemplo: `NE2450-CUV.pdf` → `CUV_NE2450.pdf`
...
with st.expander("🔍 Ejemplos de Patrones Válidos"):
    ...
    **Patrones que SÍ serán renombrados:**
    - `NE2450-CUV.pdf` → `CUV_NE2450.pdf`
    - `NE999999-CUV.xlsx` → `CUV_NE999999.xlsx`
    - `NE123456789-CUV.json` → `CUV_NE123456789.json`
    - `documento_NE8888-CUV.txt` → `CUV_NE8888.txt`
    
    **Patrones que NO serán renombrados:**
    - `factura_normal.pdf`
    - `CUV_NE2450.pdf` (ya está en formato correcto)
    - `NE2450.pdf` (falta -CUV)
    """)
    ...
    if st.button("🚀 Renombrar Archivos", type="primary", use_container_width=True):
        with st.spinner("Procesando archivos..."):
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                # Procesar archivos
                resultados, contador = renombrar_archivos_cuv_sispro(temp_dir)
                ...
                if contador > 0:
                    ...
                    zip_path = os.path.join(temp_dir, "archivos_cuv_sispro_renombrados.zip")
                    ...
                    st.download_button(
                        ...
                        data=zip_data,
                        file_name="archivos_cuv_sispro_coosalud.zip",
...
with st.expander("📖 Instrucciones de Uso"):
    st.markdown("""
    ### Cómo usar el Renombrador CUV SISPRO:
    
    1. **Selecciona archivos**: Haz clic en 'Browse files' o arrastra los archivos
    2. **Verifica patrones**: Los archivos deben tener formato `NE######-CUV`
    3. **Procesa**: Haz clic en 'Renombrar Archivos'
    4. **Descarga**: Obtén los archivos renombrados individualmente o en ZIP
    
    ### Transformación aplicada:
    - `NE2450-CUV.ext` → `CUV_NE2450.ext`
    - `NE999999-CUV.ext` → `CUV_NE999999.ext`
    
    ### Características:
    - ✅ Detecta automáticamente patrones NE######-CUV
...
# FOOTER
st.markdown("---")
st.caption("🔢 Renombrador CUV SISPRO - Coosalud • v1.0 • Conversión de patrones NE######-CUV")
