...
st.set_page_config(
    page_title="Conversor + Renombrador - Savia Salud", 
    page_icon="🔄", 
    layout="wide"
)
modern_navbar()
...
# Función de renombrado CUV ESPECÍFICA PARA SAVIA SALUD
...
def procesar_todo(directorio):
    ...
    # Procesar archivos para renombrar (CON FORMATO SAVIA)
    renombrados, contador_renombrados = renombrar_archivos_cuv_savia(directorio)
...
uploaded_files = st.file_uploader(
    "Selecciona archivos para procesar ( Archivos tipo JSON con patrón NE#)",
...
st.info("""
**🔄 Funcionalidad Combinada - Savia Salud:**
...
# Mostrar ejemplos de patrones ESPECÍFICOS DE SAVIA
with st.expander("🔍 Ejemplos de Archivos Aceptados - Savia Salud"):
    st.markdown("""
    ...
    **Archivos para Renombrar (Formato Savia - CUV al final):**
    - `NE651.pdf` → `NE651_CUV.pdf`
    - `NE999999.xlsx` → `NE999999_CUV.xlsx`
    - `documento_NE8888.txt` → `NE8888_CUV.txt`
    
    **Formato Savia vs Coosalud:**
    - 🏥 **Savia**: `NE651_CUV.pdf` (CUV al final)
    - 💊 **Coosalud**: `CUV_NE651.pdf` (CUV al inicio)
    ...
    if st.button("🚀 Procesar Todo", type="primary", use_container_width=True):
        with st.spinner("Procesando archivos JSON y renombrando archivos..."):
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                # RESULTADOS DETALLADOS - RENOMBRADO SAVIA
                if resultados['archivos_renombrados']:
                    st.subheader("🔢 Resultados Renombrado CUV - Savia")
                    ...
                    if renombrados:
                        ...
                        for resultado in renombrados:
                            ...
                            st.caption(f"Número de factura: {resultado['numero_factura']} - Formato Savia")
                ...
                if archivos_para_descargar:
                    ...
                    zip_path = os.path.join(temp_dir, "archivos_procesados_savia.zip")
                    ...
                    st.download_button(
                        ...
                        data=zip_data,
                        file_name="archivos_procesados_savia_salud.zip",
...
with st.expander("📖 Instrucciones de Uso"):
    st.markdown("""
    ### Cómo usar el Conversor + Renombrador Combinado - Savia Salud:
    ...
    **Para archivos con patrón NE###### (Formato Savia):**
    - `NE651.pdf` → `NE651_CUV.pdf` (CUV al final)
    - `NE999999.xlsx` → `NE999999_CUV.xlsx`
    - `documento_NE8888.txt` → `NE8888_CUV.txt`
    
    ### Diferencia entre EPS:
    - **Savia Salud**: `NE651_CUV.pdf` (CUV al final)
    - **Coosalud**: `CUV_NE651.pdf` (CUV al inicio)
    
    ### Características:
    - ✅ Procesamiento simultáneo de JSON y renombrado
    - ✅ Formato Savia específico para renombrado
...
# FOOTER
st.markdown("---")
st.caption("🔄 Conversor + Renombrador - Savia Salud • v2.0 • Procesamiento Combinado")
